...
try:
    ...
    print("Fetching NQ data (30 days, 1-minute)...")
    nq_data = loader.fetch_historical_bars('NQ', period='30d', interval='1m')
    
    print("Fetching ES data (30 days, 1-minute)...")
...
try:
    ...
    # Test on last 3 days
    test_dates = [
        nq_data.index[-1] - timedelta(days=2),
        nq_data.index[-1] - timedelta(days=1),
        nq_data.index[-1]
    ]
...
try:
    ...
    # Calculate ADR for today
    adr = adr_calc.calculate(nq_data, nq_data.index[-1])
    
    print(f"   20-day ADR: {adr:.2f} points")
...
try:
    ...
    ons_filter = ONSFilter(min_ratio=0.30, max_ratio=0.70, adr_lookback=20)
...
try:
    ...
    # Test on a sample move (last 10 bars)
